...
def test_create_note_invalid_json(test_app):
    response = test_app.post(
        "/api/v1/notes/", content=json.dumps({"title": "something"})
    )
    assert response.status_code == 422

    response = test_app.post(
        "/api/v1/notes/", content=json.dumps({"title": "1", "description": "2"})
    )
    assert response.status_code == 422
...
def test_read_note_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.get("/api/v1/notes/999")
    assert response.status_code == 404
    assert response.json()["detail"] == "Note not found"

    response = test_app.get("/api/v1/notes/0")
    assert response.status_code == 422
...
def test_update_note(test_app, monkeypatch):
    ...
    response = test_app.put("/api/v1/notes/1/", content=json.dumps(test_update_data))
    assert response.status_code == 200
...
def test_remove_note_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.delete("/api/v1/notes/999/")
    assert response.status_code == 404
    assert response.json()["detail"] == "Note not found"

    response = test_app.delete("/api/v1/notes/0/")
    assert response.status_code == 422
